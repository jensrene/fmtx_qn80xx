...
int main()
{
    ...
    rdsmanager m_rds(32);
    m_rds.setTransmitter(consoleTransmit,true);
    ...
    int index = m_rds.addGroup0A(m_rds.parseHex("1234"), /*TP=*/1,/*PTY=*/0x00,/*TA=*/0, /*MS=*/1, "ROCK 101", 0);
    // 'index' now should be 4, plus the sentinel at 4 => next free is 4

    // 3) Add a single 4A group (time), after that
    index = m_rds.addGroup4A(0x1234, 2025, 1, 31, 12, 0, /*offset*/2, index);
    // 'index' is now 5, plus sentinel at 5 => next free is 5

    // Let's add another 4A group (like a second time or weighting)
    index = m_rds.addGroup4A(0x1234, 2025, 1, 31, 12, 5, /*offset*/2, index);
    // 'index' is now 6, sentinel at 6 => next free is 6

    index = m_rds.addGroup2A(0x1235,0,0x00, 0,"This is my station",index);
