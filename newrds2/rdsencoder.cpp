...
rdsencoder::rdsencoder(int bufferSize)
{
    if (bufferSize > 0) {
...
void rdsencoder::setTransmitter(TransmitFunc func) {
    rdsencoder::transmit = func;
    }

// ------------------------------------------------
// clearBuffer
//  Set the very first slot to sentinel => means “empty.”
void rdsencoder::clearBuffer()
{
    if (!buffer_) return;
    for (int i = 0; i < bufferSize_; i++)
    {
...
// ------------------------------------------------
// isSentinel
bool rdsencoder::isSentinel(int index)
{
...
// ------------------------------------------------
// writeSentinel
void rdsencoder::writeSentinel(int index)
{
    if (!buffer_) return;
...
void rdsencoder::buildGroup0ASub(
    RDSMessage* outMsg,
    uint16_t piCode,
    uint8_t tp,
    uint8_t pty,
...
uint16_t rdsencoder::computeMJD(int year, int month, int day)
{
...
void rdsencoder::buildGroup2ASub(
        RDSMessage* outMsg,
        uint16_t piCode,
        uint8_t tp,
        uint8_t pty,
...
{    
    // set group type
    uint8_t group_type = rdsencoder
::group_types::RADIO_A_2A;
...
void rdsencoder::sendRDSOverI2C(const RDSMessage* msg)
{
...
uint64_t rdsencoder::getCurrentTimeMs(void) 
{
    struct timespec ts;
...
uint16_t rdsencoder::parseHex(const char* str) {
    uint16_t result = 0;
    int count = 0;
...
uint8_t rdsencoder::iso8859ToRDSChar(unsigned char c) {
